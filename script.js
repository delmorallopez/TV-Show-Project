function setup() {
  render(); // Optionally show empty page first
  fetchFilms(); // This will handle fetching, rendering, populating selector, and errors
}
...
// Fetching the episodes from the TVmaze API
const endpoint = "https://api.tvmaze.com/shows/82/episodes";
// This function sets up the initial state and fetches the episodes
// It also handles the loading message and error handling
const messageElem = document.getElementById("message");
const fetchFilms = async () => {
  messageElem.textContent = "Loading episodes..."; // Show loading message
  try {
    ...
    appState.allEpisodes = films; // Store the fetched episodes in appState
    render(); // Render the episodes after fetching
    populateEpisodeSelector(); // Populate the episode selector
    messageElem.textContent = ""; // Clear loading message
  } catch (error) {
  console.error("Failed to fetch episodes:", error);
  messageElem.textContent = "Error loading episodes. Please try again later.";
}
};
...
// This function renders the episodes based on the current appState
function render() {
  ...
  // display message how many episodes are found
...
// populateEpisodeSelector function populates the episode selector dropdown with all episodes
